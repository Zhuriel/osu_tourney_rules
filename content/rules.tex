\title{\name{} Rules}
\author{Template by Zhuriel}
\date{}

\begin{document}
\maketitle

%This document is intended to provide an easily customizable, clearly worded rule set for osu! tournaments. It is based primarily on the OWC rules, but I have attempted to give the rules better structure and clarifiy the wordings.

\section{Administrative}

\name{} (\nameshort) is a \ifthenelse{\boolean{team}}{team}{player}-based \mode{} tournament hosted by \host. \ifthenelse{\boolean{team}}{It is played in teams of \teamsize}{}
...
%%% a few examples, use what you need and adapt
Registration is handled through the form linked on the \href{\thread}{tournament thread}.
%To register, write a PM to (person) with the following template:
%To register, respond to the tournament thread with the following template:
...
\end{enumerate}
\end{samepage}
If there is still a tie that prevents determining the top 2 \ifthenelse{\boolean{team}}{teams}{players}, another match between those \ifthenelse{\boolean{team}}{teams}{players} is played to resolve the tie.
...
%Additionally, each round will include a special map pool containing maps focusing on niche skills. The detailed rules for this pool are still under consideration and will be announced when they are finalized.
...
The referee designated for the match creates the multiplayer lobby 15min before the scheduled starting time and invites the \ifthenelse{\boolean{team}}{captains, who in turn invite their team}{players}.
...
The referee keeps the host privileges of the lobby at all times except for selecting warmup maps.

\subsubsection{Absent Players}
If \ifthenelse{\boolean{team}}{not enough players are}{a player is not} present at the scheduled starting time of the match, it may be postponed a maximum of 10 minutes.
...
Each \ifthenelse{\boolean{team}}{team}{player} may choose one warmup map before the actual match. Warmup maps may not be longer than \maxwarmup{} and may not contain questionable or offensive material of any kind. The final decision regarding what is considered in breach of this rule lies with the tournament management.
...
\subsubsection{Bans}
After both warmups have been played, each \ifthenelse{\boolean{team}}{team}{player} must ban 2 maps, preventing them from being picked during the entire match.
...
%%% optional - fewer bans in group stages
%In the group stage, only one ban per \ifthenelse{\boolean{team}}{team}{player} is allowed.
...
%%% optional alternate rule to only require picking player to pick a mod
\ifthenelse{\boolean{team}}{At least 2 players per team}{
Each player
%The player who picked the map
...
Behavior during and between matches underlies the global osu! rules. If a player becomes unable to play multiplayer during a match due to a silence or restriction, \ifthenelse{\boolean{team}}{the team of that player must continue playing without them.}{that player forfeits the match.}
