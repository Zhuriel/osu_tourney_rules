...
\footnotesize Ruleset generator by Zhuriel}
...
\numparticipants{} \ifthenelse{\boolean{team}}{teams}{players} will participate in this tournament. It may be cancelled if there are not enough registrations or \ifthenelse{\boolean{team}}{teams}{players} may be excluded if there are too many registrations. If one of these cases occurs, an announcement will be made.
...
Players are responsible for their own equipment, software and connection. If any of these prevent a player from playing a match at the scheduled time and the match is not properly rescheduled, \ifthenelse{\boolean{team}}{the team must play without that player and may forfeit the match if this prevents them from having enough players present.}{it is considered a forfeit.}\ifthenelse{\boolean{scorev2}}{Additionally, as this tournament is played on score V2, players can not use the fallback release stream to play their matches}{}
